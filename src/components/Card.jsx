...
function Card(props) {
  ...
  // Toggle the favorite star state
    ...
    >
      ...
      <div className="flex flex-col justify-between p-4 leading-normal">
        ...
        <p className="mb-3 font-normal text-gray-700 dark:text-white">
          {props.description ? props.description.slice(0, cardIsOpen ? props.description.length : 70) : "No description available"}
          {cardIsOpen ? '' : '...'}
        </p>
        <div className="text-sm text-gray-600 dark:text-white">
          {cardIsOpen && (
            <>
              <strong>Ingredients:</strong> {props.ingredients} <br />
            </>
          )}
          ...
          {props.isAuthenticated && (
            <button
              onClick={(event) => {
                event.stopPropagation(); // Prevent card from opening when clicking delete
                props.onDelete();
              }}
          ...
          {cardIsOpen && (
            <div className="m-3">
              <strong>Preparation:</strong> {props.prepare} <br />
            </div>
          )}
        </div>
